...
def read_blogs_for_user(user_id: int, db: Session = Depends(get_db)):
    blogs = crud.get_blogs_by_user(db, user_id=user_id)
    return blogs

# Обновление блога
@router.put("/{user_id}/blogs/{blog_id}", response_model=schemas.Blog)
def update_blog(user_id: int, blog_id: int, blog: schemas.BlogUpdate, db: Session = Depends(get_db)):
...
def patch_blog(user_id: int, blog_id: int, blog: schemas.BlogUpdate, db: Session = Depends(get_db)):
    db_blog = crud.get_blog(db, blog_id=blog_id, user_id=user_id)
    if db_blog is None:
        raise HTTPException(status_code=404, detail="Блог не найден")
